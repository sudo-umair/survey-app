...
import { IEnumeratorSectionC7ScreenProps } from '@interfaces/screens';
...
import { SECTION_C7_QUESTIONS } from '@common/questions/survey-7';

const SectionC7Screen = ({
  navigation,
  route,
}: IEnumeratorSectionC7ScreenProps) => {
  const [questions, setQuestions] = useState<IQuestion[]>(SECTION_C7_QUESTIONS);
  ...
  const onPress = () => {
    if (checkSurveyAnswers(questions)) {
      dispatch(submitSectionC7(questions));
      dispatch(filterSurveyComponents(SURVEY_COMPONENTS.S7));
      navigation.navigate(EnumeratorScreens.SectionD7);
    }
  };
  ...
  return (
    ...
    >
      <Text style={styles.title}>Section C7: {SECTION_TITLES.sectionC}</Text>
...
export default SectionC7Screen;
